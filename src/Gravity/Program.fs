...
let main argv = 
    ...
    let initialView = 
        CameraView.lookAt (V3d(50.0, 50.0, 50.0)) V3d.Zero V3d.OOI
    let view = 
    ...
    let positions  = Mod.init (Array.create n     V3f.Zero)
    let positions' = Mod.init (Array.create (n*2) V3f.Zero)
    let colors     = Mod.init (Array.create n     C4b.Black)
    let colors'    = Mod.init (Array.create (n*2) C4b.Black)
    ...
    let sg =
        [ points; lines; aardvark; grid; transparentPlane ]
        |> Sg.group
